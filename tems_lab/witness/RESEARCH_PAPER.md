...
The theoretical design allows for three moments of Oath creation; the shipping v5.3.0 runtime uses only the first. This section distinguishes the two, and argues — from empirical evidence — that Moment 1 alone is sufficient for every execution task we have tested.

**Moment 1 — Root Oath (on user message receipt) — IMPLEMENTED AND VALIDATED.** When a non-trivial request arrives, the Planner produces a Root Oath: a paragraph of what "done" means for the whole request plus typically 5–6 top-level checkable predicates, each tied to a Tier 0 primitive. The Spec Reviewer (§6.4) rejects the Oath at zero LLM cost if it lacks a Tier 0 anchor, a wiring check, or a stub check for code-producing tasks. The accepted Oath is sealed into the Ledger *before the agent loop runs*, and the final reply is gated on `verify_oath()` immediately before `Done` is emitted. This path is wired in `AgentRuntime` via `.with_auto_planner_oath(true)` and has been validated across 73 real-LLM sessions on Gemini 3 Flash Preview and gpt-5.4, including 16 sessions on the real 152K-LOC TEMM1E codebase. In every observed case, a single Root Oath was sufficient to either verify a clean completion or catch a partial one.

**Moment 2 — Subtask Oaths (just-in-time, as decomposition reveals structure) — TYPE-SUPPORTED, RUNTIME WIRING DEFERRED.** In the original design, each subtask's Oath was to be sealed immediately before that subtask's first tool call, with the Root Oath remaining the anchor: `root_goal_met()` is true only when all subtask verdicts plus the Root's own postconditions pass. The `temm1e-witness` crate carries the full type skeleton for this design — `SubtaskId`, `RootGoalId`, `Oath.subtask_id`, `Oath.root_goal_id` — and the Ledger knows how to persist multiple sealed Oaths per session. The runtime path that *triggers* a mid-loop seal, however, is not wired in v5.3.0. We explain why below.

**Moment 3 — Skip Requests — TYPE-SUPPORTED, RUNTIME WIRING DEFERRED.** The `SkipRequested` and `SkipApproved` ledger entries and `WitnessSubTaskStatus` variants exist. The agent-facing tool that files a skip and the Witness-side LLM call that decides `SkipApproved` vs `SkipDenied` based on coherence with the Root Oath do not. Same rationale as Moment 2.

#### Why Moment 1 alone is sufficient: end-state verification vs path verification

The critical architectural property of Witness is that it verifies *end state*, not *path*. A Root Oath of the form "after the agent runs, file X must exist, function Y must be called in ≥N places, pattern Z must not appear, TODO markers must be absent" does not care *how* the agent reached that state. The agent may explore the codebase for eight tool calls before touching anything; may take a wrong path and backtrack; may discover call sites it did not anticipate; may silently skip a subtask it later judges unnecessary. **None of these intermediate decisions affect the verdict.** The end state either satisfies the predicates or it does not. This is the same property that makes contract-based formal methods work: the specification pins down the output, not the process.

The concern that motivated Moments 2–3 in the original design was that "nobody can know the full scope before investigation." Empirically, this turns out to be a misframing. The Planner does not need to know the full scope of *intermediate work*; it only needs to specify the *end state*. For every execution task we have run on the Tem codebase — rename refactors, cross-crate symbol changes, documentation sweeps — the end state is perfectly specifiable at t=0 even when the path to reach it is not. The exploration happens inside the agent loop; the verification only looks at the filesystem afterward.

#### The empirical evidence

In 16 real-LLM sessions on the TEMM1E codebase spanning Phases 4, 5, and 6, a single Root Oath with 5–6 Planner-generated postconditions was sufficient in every case: it caught the first real-LLM partial completion (Phase 4, Gemini, file 22% smaller than expected), logged the first real-LLM full PASS verdict (Phase 5, gpt-5.4, 6/6 postconditions on a hard rename refactor), and fired every wiring branch correctly in the Phase 6 live validator. We observed **zero cases** where the Root Oath was insufficient and a JIT subtask oath would have caught something the Root did not. Full results in `EXPERIMENT_REPORT.md` §13–§15.

This is not yet a sample size that proves JIT oaths are unnecessary in general; it is a sample size that justifies *deferring the runtime wiring until empirical evidence creates the calibration need*. Adding JIT today would introduce ~850 LOC of runtime machinery, N+1 Planner LLM calls per session (3–10× cost and latency increase), and N additional Spec-Reviewer surface areas on Oaths that the Planner authors with less context than the Root — while, in the tasks we have observed, catching nothing the existing path does not already catch. The design tradeoff is unambiguously negative today and may become positive when we encounter execution tasks the Root Oath cannot specify.

#### When Moments 2–3 would become necessary

The design we deferred would become load-bearing in three specific conditions:

1. **A real-LLM execution task on the Tem codebase (or any production repo) where a Root Oath with the maximum sensible postcondition count still allowed the agent to ship a silent partial completion.** This is the case we have not yet seen; Phase 7 work begins the moment we see one.
2. **Multi-hour, multi-phase tasks whose intermediate artifacts become inputs to later subtasks.** A task like *"split crate X into sub-crates A, B, C, then migrate callers to the new paths"* has ordered subtasks where subtask N depends on subtask N−1 being complete. The Root Oath can still specify the end state, but mid-loop verification of subtask N−1 before subtask N begins would prevent cascading wasted work.
3. **Pure exploration/analysis tasks** ("audit this repo", "profile this service") where there is no end state to verify because the output is a report whose quality depends on what was discovered. These are a different task class entirely — agents do not typically lie about completion on analysis tasks the way they do on execution tasks — and we flag them as a separate research direction rather than a Witness extension.

Until one of those conditions materializes in empirical data, Moment 1 remains the production path and Moments 2–3 remain a documented research direction with type support in place. The Ledger schema, `SubtaskId`/`RootGoalId` types, and `SkipRequested`/`SkipApproved` entry types stay in the crate; they cost nothing to maintain and preserve the option to implement Phase 7 when the evidence demands it.
